...
module.exports.run = async (bot, message, splitMessage) => {
  ...
  const voiceConnection = bot.voiceConnections.find(val => val.channel.guild.id ==
    message.guild.id);
  if(voiceConnection === null) return functions.sendError(message,
    'Aucune musique n\'est jouée');
  const dispatcher = voiceConnection.player.dispatcher;
  //volume
  if(!(splitMessage.length === 2)) return;
  if(isNaN(splitMessage[1])) {
    return functions.sendError(message, "Indiquer le volume souhaité: " +
      config.prefix + "volume <numéro>");
  }
  if((splitMessage[1] > 100) || (splitMessage[1] < 0)) return functions.sendError(
    message, 'Volume en dehors des bornes 0..100');
  functions.sendEmbed(message, 'Volume défini sur: ' + splitMessage[1],
    'send', true);
