  ...
  if(splitMessage.length === 1) {
    if(config.typeOfActivity.toLowerCase() === 'streaming') {
      functions.sendEmbed(message,
        `L\'activité actuel du bot est: \` ${config.activity} \` en mode \`${config.typeOfActivity}\` sur la chaine: \`${config.StreamURL}\``,
        'send', true);
    } else
      functions.sendEmbed(message,
        `L\'activité actuel du bot est: \` ${config.activity} \` en mode \`${config.typeOfActivity}\``,
        'send', true);
  } else {
    ...
    functions.sendEmbed(message, 'L\'activité du bot a été changée sur: ' +
      newActivity + 'en mode ' + newTypeOfActivity, 'send', true);
    bot.user.setActivity(config.activity + " | " + config.prefix + "help", {
      type: config.typeOfActivity,
