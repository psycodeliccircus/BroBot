  ...
  if(message.author.id === config.ownerID) {
    if(splitMessage.length === 1) {
      functions.sendEmbed(message,
        `Le channel bot actuel est: \`${config.salonBot} \``, 'send', true);
    } else {
      let newChannel = message.mentions.channels.first()
        .name;
      let newChannelId = message.mentions.channels.first()
        .id;
      ...
      functions.sendEmbed(message,
        `Le channel bot a été défini sur: \`${newChannel}\``, 'send', true);
